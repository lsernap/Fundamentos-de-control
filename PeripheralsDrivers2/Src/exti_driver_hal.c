...
static void exti_enable_clock_peripheral(EXTI_Config_t *extiConfig);
...
void exti_Config(EXTI_Config_t *extiConfig){
	...
	exti_enable_clock_peripheral(extiConfig);
...
static void exti_assign_channel(EXTI_Config_t *extiConfig){
	...
	case 0: {
			...
			if (extiConfig->pGPIOHandler->GPIOx == GPIOA) {
				SYSCFG->EXTICR[0] |= (SYSCFG_EXTICR1_EXTI0_PA);

			} else if (extiConfig->pGPIOHandler->GPIOx == GPIOB) {
				SYSCFG->EXTICR[0] |= (SYSCFG_EXTICR1_EXTI0_PB);

			} else if (extiConfig->pGPIOHandler->GPIOx == GPIOC) {
				SYSCFG->EXTICR[0] |= (SYSCFG_EXTICR1_EXTI0_PC);

			} else if (extiConfig->pGPIOHandler->GPIOx == GPIOD) {
				SYSCFG->EXTICR[0] |= (SYSCFG_EXTICR1_EXTI0_PD);

			} else if (extiConfig->pGPIOHandler->GPIOx == GPIOE) {
				SYSCFG->EXTICR[0] |= (SYSCFG_EXTICR1_EXTI0_PE);

			} else if (extiConfig->pGPIOHandler->GPIOx == GPIOH) {
...
static void exti_select_edge(EXTI_Config_t *extiConfig){
	...
	if(extiConfig->edgeType == EXTERNAL_INTERRUPT_FALLING_EDGE){
		...
		EXTI->FTSR |= (0x1 << (extiConfig->pGPIOHandler->pinConfig.GPIO_PinNumber));
	}
	else{
		EXTI->RTSR |= (0x1 << (extiConfig->pGPIOHandler->pinConfig.GPIO_PinNumber));
	}
}
...
static void exti_config_interrupt(EXTI_Config_t *extiConfig){
	/* 6.0 Activamos la interrupción del canal que estamos configurando */
		...
		EXTI->IMR |= (0x1 << (extiConfig->pGPIOHandler->pinConfig.GPIO_PinNumber));
